...
class DataStoreManager(private val context: Context) {
  companion object {
    ...
    private val READER_ID_KEY = stringPreferencesKey("reader_id")
  }

  suspend fun storeReaderId(readerId: String) {
    context.appConfigurations.edit { preferences -> preferences[READER_ID_KEY] = readerId }
  }

  fun getReaderIdFlow(): Flow<String> {
    val readerIdFlow: Flow<String> =
      context.appConfigurations.data
        .map { preferences -> preferences[READER_ID_KEY] ?: "" }
    return readerIdFlow
  }
}
